...
class PerformanceValidator:
    ...
    def __init__(self):
        ...
        self.performance_targets = {
            'neural': {
                'neural_lz': {'ratio_min': 1.5, 'ratio_max': 3.0, 'latency_max': 0.001},
                'arithmetic': {'ratio_min': 2.0, 'ratio_max': 4.0, 'latency_max': 0.001},
                'perceptual': {'ratio_min': 2.0, 'ratio_max': 10.0, 'latency_max': 0.001},
                'predictive': {'ratio_min': 1.5, 'ratio_max': 2.0, 'latency_max': 0.002},
            },
            'emg': {
                'emg_lz': {'ratio_min': 5.0, 'ratio_max': 12.0, 'latency_max': 0.025},
                'emg_perceptual': {'ratio_min': 8.0, 'ratio_max': 20.0, 'latency_max': 0.035},
                'emg_predictive': {'ratio_min': 10.0, 'ratio_max': 25.0, 'latency_max': 0.050},
                'mobile_emg': {'ratio_min': 3.0, 'ratio_max': 8.0, 'latency_max': 0.015},
            }
        }
    ...
    def validate_neural_algorithms(self) -> Dict[str, Any]:
        ...
        try:
            ...
            compression_ratio = metadata.get(
                ...
                neural_data.nbytes / len(compressed) if isinstance(compressed, bytes) else 1.0
            )

            # Validate against targets
            targets = self.performance_targets['neural']['neural_lz']
            ratio_valid = targets['ratio_min'] <= compression_ratio <= targets['ratio_max']
            ...
            logger.info(f"Neural LZ: {compression_ratio:.2f}x ratio, {compression_time * 1000:.2f}ms - {results['neural_lz']['status']}")
        ...
        try:
            ...
            results['perceptual'] = {
                ...
                'status': 'PASS' if (ratio_valid and latency_valid and snr_valid) else 'FAIL'
            }

            logger.info(f"Perceptual: {compression_ratio:.2f}x ratio, {snr:.1f}dB SNR, {quantization_time * 1000:.2f}ms - {results['perceptual']['status']}")
    ...
    def validate_emg_algorithms(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results['emg_lz'] = {
                ...
                'ratio_meets_target': targets['ratio_min'] <= compression_ratio <= targets['ratio_max'],
                'latency_meets_target': compression_time <= targets['latency_max'],
                'quality_meets_target': quality_score >= 0.85,
                'status': 'PASS' if (targets['ratio_min'] <= compression_ratio <= targets['ratio_max']
                                   and compression_time <= targets['latency_max']
                                   and quality_score >= 0.85) else 'FAIL'
            }

            logger.info(f"EMG LZ: {compression_ratio:.2f}x ratio, Q={quality_score:.3f}, {compression_time*1000:.2f}ms - {results['emg_lz']['status']}")
        ...
        try:
            ...
            results['emg_perceptual'] = {
                ...
                'ratio_meets_target': targets['ratio_min'] <= compression_ratio <= targets['ratio_max'],
                'latency_meets_target': compression_time <= targets['latency_max'],
                'quality_meets_target': quality_score >= 0.90,
                'status': 'PASS' if (targets['ratio_min'] <= compression_ratio <= targets['ratio_max']
                                   and compression_time <= targets['latency_max']
                                   and quality_score >= 0.90) else 'FAIL'
            }

            logger.info(f"EMG Perceptual: {compression_ratio:.2f}x ratio, Q={quality_score:.3f}, {compression_time*1000:.2f}ms - {results['emg_perceptual']['status']}")
        ...
        try:
            ...
            decompressed = compressor.decompress(compressed)
            quality_score = self._calculate_emg_quality(mobile_emg_data, decompressed)
            ...
            results['mobile_emg'] = {
                ...
                'ratio_meets_target': targets['ratio_min'] <= compression_ratio <= targets['ratio_max'],
                'latency_meets_target': compression_time <= targets['latency_max'],
                'quality_meets_target': quality_score >= 0.80,
                'status': 'PASS' if (targets['ratio_min'] <= compression_ratio <= targets['ratio_max']
                                   and compression_time <= targets['latency_max']
                                   and quality_score >= 0.80) else 'FAIL'
            }

            logger.info(f"Mobile EMG: {compression_ratio:.2f}x ratio, Q={quality_score:.3f}, {compression_time*1000:.2f}ms - {results['mobile_emg']['status']}")
    ...
    def validate_quality_metrics(self) -> Dict[str, Any]:
        ...
        try:
            from bci_compression.metrics.emg_quality import EMGQualityMetrics, evaluate_emg_compression_quality
            ...
            activation_metrics = quality_metrics.muscle_activation_detection_accuracy(emg_data, degraded)
            envelope_metrics = quality_metrics.emg_envelope_correlation(emg_data, degraded)
            spectral_metrics = quality_metrics.emg_spectral_fidelity(emg_data, degraded)
            timing_metrics = quality_metrics.emg_timing_precision(emg_data, degraded)

            # Test overall evaluation
            overall_quality = evaluate_emg_compression_quality(emg_data, degraded, 2000.0)

            results['emg_quality_metrics'] = {
                'activation_precision': activation_metrics.get('activation_precision', 0),
                'activation_recall': activation_metrics.get('activation_recall', 0),
                'activation_f1': activation_metrics.get('activation_f1', 0),
                'envelope_correlation': envelope_metrics.get('envelope_correlation', 0),
                'spectral_correlation': spectral_metrics.get('spectral_correlation', 0),
                'timing_accuracy': timing_metrics.get('timing_accuracy', 0),
                'overall_quality': overall_quality.get('overall_quality_score', 0),
                'status': 'PASS'
            }

            logger.info(f"EMG Quality Metrics: All tests passed")
    ...
    def validate_plugin_system(self) -> Dict[str, Any]:
        ...
        try:
            from bci_compression.algorithms.emg_plugins import get_emg_compressors, create_emg_compressor
            ...
            for plugin_name in emg_compressors:
                try:
                    ...
                    plugin_results[plugin_name] = {'error': str(e), 'status': 'ERROR'}

            results['plugin_functionality'] = plugin_results

            logger.info(f"Plugin System: {len([p for p in plugin_results.values() if p.get('status') == 'PASS'])}/{len(plugin_results)} plugins working")
    ...
    def validate_mobile_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            scenarios = [
                {'battery_level': 0.8, 'cpu_usage': 0.3, 'description': 'High battery'},
                {'battery_level': 0.4, 'cpu_usage': 0.7, 'description': 'Medium battery'},
                {'battery_level': 0.1, 'cpu_usage': 0.5, 'description': 'Low battery'}
            ]
            ...
            for i, scenario in enumerate(scenarios):
                try:
                    ...
                    scenario_results[f"scenario_{i}"] = {
                        ...
                        'recommended_sampling_rate': config.get('sampling_rate_hz', 0),
                        'compression_level': config.get('compression_level', 0),
                        'processing_interval': config.get('processing_interval_ms', 0),
                        'valid_config': all(k in config for k in ['sampling_rate_hz', 'compression_level', 'processing_interval_ms']),
                        'status': 'PASS'
                    }

                except Exception as e:
                    scenario_results[f"scenario_{i}"] = {'error': str(e), 'status': 'ERROR'}

            results['power_optimization'] = scenario_results

            # Validate that low battery scenarios recommend lower sampling rates
            high_battery_sr = scenario_results.get('scenario_0', {}).get('recommended_sampling_rate', 0)
            low_battery_sr = scenario_results.get('scenario_2', {}).get('recommended_sampling_rate', 0)

            results['power_adaptation'] = {
                'high_battery_sampling_rate': high_battery_sr,
                'low_battery_sampling_rate': low_battery_sr,
                'power_aware_adaptation': low_battery_sr < high_battery_sr if (high_battery_sr > 0 and low_battery_sr > 0) else False,
                'status': 'PASS' if (low_battery_sr < high_battery_sr and low_battery_sr > 0) else 'FAIL'
            }

            logger.info(f"Mobile Optimization: Power adaptation working correctly")
    ...
    def _generate_realistic_neural_data(self) -> np.ndarray:
        ...
        for ch in range(n_channels):
            # Random spike times
            spike_times = np.random.choice(n_samples, size=np.random.randint(50, 200), replace=False)
            for spike_time in spike_times:
                if spike_time < n_samples - 10:
                    # Add spike waveform
                    spike_waveform = 100 * np.exp(-np.arange(10) / 3) * np.random.choice([-1, 1])
    ...
    def _generate_realistic_emg_data(self) -> np.ndarray:
        ...
        for ch in range(n_channels):
            ...
            if np.any(activation_mask):
                ...
                activation_signal = 0.3 * (ch + 1) * np.random.normal(0, 1, n_samples)
                # Add frequency content
                freq_content = 0.2 * np.sin(2 * np.pi * (100 + ch * 25) * time)
                # Apply envelope
                envelope = np.exp(-((time - (activation_start + activation_end) / 2) ** 2) / 0.05)
    ...
    def _calculate_emg_quality(self, original: np.ndarray, reconstructed: np.ndarray) -> float:
        ...
        try:
            ...
            normalized_error = rms_error / rms_original if rms_original > 0 else 1.0

            # Combined quality score
            quality_score = correlation * (1 - normalized_error) if not np.isnan(correlation) else 0.5
...
class StressTestValidator:
    ...
    def run_memory_stress_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sizes = [(256, 30000), (512, 60000), (1024, 120000)]  # channels x samples

            for i, (n_channels, n_samples) in enumerate(sizes):
                try:
                    # Generate large dataset
                    large_data = np.random.randn(n_channels, n_samples).astype(np.float32)
                    ...
                    results[f'size_{i}'] = {
                        ...
                        'memory_efficient': compression_time < 60.0,  # Should complete within 1 minute
                        'status': 'PASS' if compression_time < 60.0 else 'TIMEOUT'
                    }

                    logger.info(f"Memory test {i}: {data_size_mb:.1f}MB in {compression_time:.2f}s - {results[f'size_{i}']['status']}")

                    # Clean up
                    del large_data, compressed

                except Exception as e:
                    results[f'size_{i}'] = {'error': str(e), 'status': 'ERROR'}
                    logger.error(f"Memory stress test {i} failed: {e}")

        except Exception as e:
            results['memory_stress'] = {'error': str(e), 'status': 'ERROR'}
    ...
    def run_latency_stress_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for chunk_size in chunk_sizes:
                ...
                logger.info(f"Latency test chunk {chunk_size}: {avg_latency*1000:.2f}ms avg - {results[f'chunk_{chunk_size}']['status']}")

            # Overall latency performance
            results['overall_latency'] = {
                'avg_latency_ms': np.mean(latencies) * 1000,
                'consistent_performance': np.std(latencies) < 0.005,  # Low variance
            ...
            for test_name, test_data in edge_cases:
                try:
                    ...
                    if test_name in ['inf_values', 'nan_values']:
                        ...
                        compressed = compressor.compress(test_data)
                        results[test_name] = {'status': 'UNEXPECTED_SUCCESS', 'warning': 'Should handle inf/nan values'}
                    else:
                        compressed = compressor.compress(test_data)
                        decompressed = compressor.decompress(compressed)
                    ...
                    if test_name in ['inf_values', 'nan_values']:
                        results[test_name] = {'status': 'EXPECTED_ERROR', 'error': str(e)}
                    else:
                        results[test_name] = {'status': 'UNEXPECTED_ERROR', 'error': str(e)}
...
def run_comprehensive_validation():
    ...
    validation_results = {
        ...
        'memory_stress': stress_validator.run_memory_stress_test(),
    ...
    logger.info(f"Validation results saved to {results_dir / 'comprehensive_validation_results.json'}")
